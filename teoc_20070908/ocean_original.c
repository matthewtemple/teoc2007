...
#include "inferno/embryo_support.h"
#include "inferno/phenom.h"
...
#define FSCORE_METHOD_EXTERNAL_FITNESS 3

#define USE_FSCORE 1
#define FSCORE_METHOD FSCORE_METHOD_GENE
...
float fitness_of_organism(organism_t *organism, struct phenom_t *phenom);
organism_t *fittest_organism(world_t *world, struct phenom_t *phenom);
...
void iterate_organism(world_t *world, organism_t *organism,
		struct phenom_t *phenom);
void iterate_world(world_t *world, struct phenom_t *phenom);
...
float fitness_of_organism(organism_t *organism, struct phenom_t *phenom)
{
	...
	struct embryo_t *embryo;
	struct object_t *object;
	unsigned int each_data_object;
	unsigned int correct_classifications = 0;
	unsigned int classification;
	unsigned int decision_attribute_value;
	float fitness;

	genome = genome_create(GENOME_LENGTH, organism->genome);
	embryo = embryo_create("attero", genome, GENOME_ADDRESS_SIZE,
			attero_classify, "attero_classify");

	for (each_data_object = 0; each_data_object < phenom->object_count;
		 each_data_object++) {
		object = object_create(phenom->property_count,
				phenom->objects[each_data_object]->properties);
		initialize_embryo_memory(embryo, object);
		classification = embryo->object_classification_function(embryo,
				object);
		object_destroy(object);
		decision_attribute_value
			= object_get_property(phenom->objects[each_data_object], 0);
		if (classification == decision_attribute_value) {
			correct_classifications++;
		}
	}

	embryo_destroy(embryo);

	fitness = ((float) correct_classifications)
		/ ((float) phenom->object_count);
...
organism_t *fittest_organism(world_t *world, struct phenom_t *phenom)
{
	...
	organism = world->organisms[0][0];
	fittest_fitness = fitness_of_organism(organism, phenom);
	for (each_x = 0; each_x < WORLD_WIDTH; each_x++) {
		for (each_y = 0; each_y < WORLD_HEIGHT; each_y++) {
			organism = world->organisms[each_x][each_y];
			fitness = fitness_of_organism(organism, phenom);
		...
		struct phenom_t *phenom)
{
	...
	if (DO_MEET) {
		meet_organism(world, organism, phenom);
	}
...
void iterate_world(world_t *world, struct phenom_t *phenom)
{
	...
	for (each_x = 0; each_x < WORLD_WIDTH; each_x++) {
		for (each_y = 0; each_y < WORLD_HEIGHT; each_y++) {
			iterate_organism(world, world->organisms[each_x][each_y], phenom);
		}
	}
}
		...
		struct phenom_t *phenom)
{
	...
	float fscore_this_float;
	float fscore_other_float;
	...
	if (USE_FSCORE) {
		...
		else if (FSCORE_METHOD == FSCORE_METHOD_BIT_HISTORY) {
			fscore_this = unsigned_int_from_bit_history(organism);
			fscore_other
				= unsigned_int_from_bit_history(organism_to_exchange_with);
		}
		else if (FSCORE_METHOD == FSCORE_METHOD_EXTERNAL_FITNESS) {
			fscore_this_float = fitness_of_organism(organism, phenom);
			fscore_other_float
				= fitness_of_organism(organism_to_exchange_with, phenom);
			fscore_this = (unsigned int) (fscore_this_float * 1000000);
			fscore_other = (unsigned int) (fscore_other_float * 1000000);
		}
...
int main(int argc, char *argv[])
{
	...
	world = new_world();
	phenom = phenom_create_from_ascii_binary(
		"inferno/mushroom.data.binary.2000", 0);
	...
	for (each_iteration = 0; each_iteration < ITERATIONS; each_iteration++) {
		...
		/*
		fittest = fittest_organism(world, phenom);
		fittest_fitness = fitness_of_organism(fittest, phenom);
		printf("fitness of fittest : %f\n", fittest_fitness);
		*/

		iterate_world(world, phenom);
	}
